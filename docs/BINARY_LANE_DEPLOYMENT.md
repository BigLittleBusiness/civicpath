...
4. From `/var/www/civicpath/api`, run `node db/migrations/002_system_admin_foundation.mjs`, `node db/migrations/003_customer_360_and_mfa.mjs`, `node db/migrations/004_structured_selectors.mjs` and `node db/migrations/005_public_pulse_leads.mjs`. These migrations are idempotent; retain them in the deployment record.

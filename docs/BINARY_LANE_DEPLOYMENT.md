...
3. Create `/var/www/civicpath/api/.env` with `NODE_ENV=production`, a unique high-entropy `JWT_SECRET`, distinct high-entropy `PLATFORM_ENCRYPTION_KEY` and `ALTCHA_HMAC_SECRET` values, the production database settings, `FRONTEND_URL=https://app.civicpath.com.au`, `CORS_ORIGINS=https://app.civicpath.com.au,https://www.civicpath.com.au`, `COOKIE_DOMAIN=app.civicpath.com.au`, `MFA_ISSUER=CivicPath`, and `PRIVILEGED_ACTION_MINUTES=10` (or another explicitly approved short duration). `ALTCHA_HMAC_SECRET` signs short-lived, single-use, self-hosted anti-spam challenges and must never be exposed to a browser.
4. From `/var/www/civicpath/api`, run `node db/migrations/002_system_admin_foundation.mjs`, `node db/migrations/003_customer_360_and_mfa.mjs`, `node db/migrations/004_structured_selectors.mjs`, `node db/migrations/005_public_pulse_leads.mjs` and `node db/migrations/006_public_contact_and_altcha.mjs`. These migrations are idempotent; retain them in the deployment record.

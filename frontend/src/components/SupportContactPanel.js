...
export function SupportContactPanel() {
  ...
  return <section className="settings-card support-contact-card"><div><p className="card-eyebrow">Support</p><h3>Send a support enquiry</h3><p className="card-copy">Use this protected form to request help. Your enquiry is recorded in your CivicPath workspace and securely routed to the support team.</p></div><form className="support-contact-form" onSubmit={submit} noValidate aria-busy={submitting}><div className="field-grid"><label>Category<select value={form.category} onChange={update('category')} disabled={submitting}>{categories.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><small className="support-subject-preview">Email subject: {subjectPreview}</small></label><label>Subject<input value={form.subject} onChange={update('subject')} minLength="3" maxLength="240" required placeholder="What do you need help with?" disabled={submitting}/></label><label className="support-message">What happened, and what were you trying to do?<textarea value={form.message} onChange={update('message')} minLength="10" maxLength="3000" required placeholder="Include enough context for a useful response. Do not include passwords, access codes or payment details." disabled={submitting}/></label><label className="support-honeypot" aria-hidden="true">Leave this field blank<input tabIndex="-1" autoComplete="off" value={form.honeypot} onChange={update('honeypot')}/></label></div><AltchaVerification key={verificationKey} challengeUrl="/v1/support/contact-challenge" onVerified={(value) => setForm((current) => ({ ...current, altcha: value }))}/>{successMessage && <div className="support-submission-success" role="status" aria-live="polite"><span aria-hidden="true">✓</span><div><b>Support enquiry recorded</b><p>{successMessage}</p></div></div>}<div className="support-contact-actions"><small>Every support enquiry requires a verification check and remains visible in the platform support queue.</small><div className="support-submit-stack">{submitting && <span className="support-submitting-status" role="status" aria-live="polite"><i aria-hidden="true"/>Sending securely…</span>}<button className="primary-button" type="submit" disabled={submitting || !form.altcha}>{submitting ? <><i className="button-spinner" aria-hidden="true"/>Sending enquiry…</> : 'Send support enquiry'}</button></div></div></form></section>;
}

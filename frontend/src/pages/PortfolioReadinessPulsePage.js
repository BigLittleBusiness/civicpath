...
export default function PortfolioReadinessPulsePage() {
  ...
  const [form, setForm] = useState({ sessionId: newSessionId(), country: 'AU', portfolioFocus: [], constraintThemes: [], marketingConsent: false, resourceAcknowledged: false, altcha: '', honeypot: '', sourceUrl: window.location.href, referrer: document.referrer || '' });
  ...
  const currentComplete = step === 0 ? contextComplete : step === 1 ? questionComplete(questionGroups.slice(0, 4)) : step === 2 ? readinessComplete : Boolean(form.firstName && form.lastName && form.email && form.resourceAcknowledged && form.altcha);
